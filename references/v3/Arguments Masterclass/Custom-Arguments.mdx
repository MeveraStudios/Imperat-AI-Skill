...
Imperat comes shipped with built-in support for native java types, an even specific types per platform.
For more details on this, check out [Supported-Types](docs/Imperat/Supported%20Types)
...
To handle errors, create a custom exception extending `SelfHandlingException`:
```java
public class UnknownRankException extends SelfHandlingException {
    ...
    @Override
    public <S extends CommandSource> void handle(
...
:::info
Learn more about error handling and custom exceptions in the [Error Handling](/docs/Imperat/Exceptions%20Guide) section.
:::
...
Learn more about suggestions in the [Suggestions](/docs/Imperat/Suggestions) section.
:::
